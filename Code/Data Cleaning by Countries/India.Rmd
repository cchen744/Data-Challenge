...
###India_ev_data
```{r}
India_ev_data <- ev_data %>% 
  filter(region=="India") %>% 
  select(-category,-mode,-X,-unit)%>% 
  arrange(parameter,powertrain) %>% 
  rename("Country"="region")

India_ev_data_by_sale <- India_ev_data %>% 
  filter(parameter == "EV sales")
```

###Korea_electricity_production
```{r}
India_electricity_production <- energy_production %>% 
  filter(Country=="India",Product=="Electricity") %>% 
  select(-X) %>% 
...
### Merge
```{r}
India_merged <-India_ev_data_by_sale %>% 
  left_join(India_ev_chargers,by=c("Country","year")) %>% 
  left_join(India_electricity_production,by=c("Country","year")) %>% 
...
```{r}
India_metric <- India_merged %>% 
  group_by(year) %>% 
  reframe(total_ev_sales_per_year = sum(`# of EV`),
            total_charging_points_per_year = sum(`# of charging station`) ) %>% 
  mutate(total_electricity_per_year = India_electricity_production$Total_electricity_year,
         influence_metric = total_electricity_per_year * total_ev_sales_per_year / total_charging_points_per_year)

write.csv(India_metric,"~/Desktop/Data Challenge/Data Challenge/CODE/Metrics/India_metric.csv",row.names = FALSE)
```
