...
###china_ev_data
```{r}
china_ev_data <- ev_data %>% 
  filter(region=="China") %>% 
  select(-category,-mode,-X,-unit)%>% 
  arrange(parameter,powertrain) %>% 
  rename("Country"="region")

china_ev_data_by_sale <- china_ev_data %>% 
  filter(parameter %in% c("EV sales", "EV stock"))

china_ev_data_by_share <- china_ev_data %>% 
  filter(parameter %in% c("EV stock share", "EV sales share")) # break down data by units (vehicles | percentage)

EV_sale_p1 <- china_ev_data_by_sale %>% filter(parameter == "EV sales") %>% 
  ggplot(aes(x = year, y = value, color = powertrain, group = powertrain)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "EV Sales by Year (China)", x = "Year", y = "Sales (Units)", color = "Powertrain") +
  theme_minimal()

EV_stock_p2 <- china_ev_data_by_sale %>% filter(parameter == "EV stock") %>% 
  ggplot(aes(x = year, y = value, color = powertrain, group = powertrain)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "EV Stocks by Year (China)", x = "Year", y = "Stocks (Units)", color = "Powertrain") +
  theme_minimal()

grid.arrange(EV_sale_p1, EV_stock_p2, nrow = 2)
```

### china_electricity_production
```{r}

energy_production <- energy_production %>%
  mutate(Country = ifelse(Country == "People's Republic of China", "China", Country))

china_electricity_production <- energy_production %>% 
  filter(Country=="China",Product=="Electricity") %>% 
  select(-X) %>% 
  ...
  group_by(Time) %>% 
  summarize(Country = "China",Total_electricity_year = sum(energy_production),Unit = 'GWh') %>% rename("year"="Time")

china_electricity_production %>% ggplot( aes(x = year, y = Total_electricity_year)) +
  geom_line(color = "lightseagreen", size = 1) +      
  geom_point(size = 2, color = "red") +     
  geom_text(aes(label = round(Total_electricity_year, 1)), vjust = -0.5, size = 3) +  
  labs(title = "China's Annual Electricity Production (2010-2021)", 
       x = "Year", 
...
china_ev_chargers %>% 
  ...
  geom_line()+
  labs(title = "China's Charging Stations by Powertrain (Slow vs Fast)",
       x = "Year",
       y = "Number of Charging Stations",
       fill = "Powertrain",
       color = "Powertrain") +
  theme_minimal()+
  geom_text(data = china_ev_chargers %>% filter(powertrain == "Publicly available fast"),
            aes(label = value), vjust = -0.5, color = "red", size = 4)
```

### Merge
```{r}
china_merged <- china_ev_data_by_sale %>% 
  inner_join(china_ev_chargers,by=c("Country","year")) %>% 
  inner_join(china_electricity_production,by=c("Country","year")) %>% 
...
```{r}
China_metric <- china_merged %>% 
  group_by(year) %>% 
  reframe(total_ev_sales_per_year = sum(`# of EV`),
            total_charging_points_per_year = sum(`# of charging station`) ) %>% 
  mutate(total_electricity_per_year = china_electricity_production$Total_electricity_year,
         influence_metric = total_electricity_per_year * total_ev_sales_per_year / total_charging_points_per_year)

write.csv(China_metric,"~/Desktop/Data Challenge/Data Challenge/CODE/Metrics/China_metric.csv",row.names = FALSE)
```
...
```{r}
ggplot(filter(china_merged, `EV type` == "BEV" | `EV type` == "PHEV"), 
       aes(x = `# of charging station`, y = `# of EV`,color = `EV type`)) +
  geom_point() +
  geom_smooth(method = "lm", se = T,color="orange") + # choose linear method temporarily
  labs(title = "EV Sales vs Charging Stations (China)", x = "Number of Charging Stations", y = "EV Sales") +
  theme_minimal()
```
...
ggplot(filter(china_merged, `EV type` == "BEV" | `EV type` == "PHEV"), 
  ...
  labs(title = "EV Sales vs Electricity Production (China)", x = "Electricity Production (GWh)", y = "EV Sales") +
  theme_minimal()
```
# Correlation Analysis
```{r}
cor_matrix <- cor(filter(china_merged, `EV type` == "BEV" | `EV type` == "PHEV") %>% select(`# of EV`, `# of charging station`, Total_electricity_year), use = "complete.obs")
print(cor_matrix)
```

# Regression Analysis
```{r}
model <- lm(`# of EV` ~ `# of charging station` + Total_electricity_year, data = filter(china_merged, `EV type` == "BEV" | `EV type` == "PHEV"))
summary(model)
```
