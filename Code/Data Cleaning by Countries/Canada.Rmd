...
###Canada_ev_data
```{r}
Canada_ev_data <- ev_data %>% 
  filter(region=="Canada") %>% 
  select(-category,-mode,-X,-unit)%>% 
  arrange(parameter,powertrain) %>% 
  rename("Country"="region")

Canada_ev_data_by_sale <- Canada_ev_data %>% 
  filter(parameter %in% c("EV sales", "EV stock"))

Canada_ev_data_by_share <- Canada_ev_data %>% 
  filter(parameter %in% c("EV stock share", "EV sales share")) # break down data by units (vehicles | percentage)

EV_sale_p1 <- Canada_ev_data_by_sale %>% filter(parameter == "EV sales") %>% 
  ggplot(aes(x = year, y = value, color = powertrain, group = powertrain)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Canada's EV Sales by Year", x = "Year", y = "Sales (Units)", color = "Powertrain") +
  geom_text(data = Canada_ev_data_by_sale %>% filter(powertrain == "FCEV"), 
            aes(label = value), 
            vjust = -1.5, color = "red", size = 3.5)+
  theme_minimal()

EV_stock_p2 <- Canada_ev_data_by_sale %>% filter(parameter == "EV stock") %>% 
  ggplot(aes(x = year, y = value, color = powertrain, group = powertrain)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Canada's EV Stocks by Year", x = "Year", y = "Stocks (Units)", color = "Powertrain") +
  geom_text(data = Canada_ev_data_by_sale %>% filter(powertrain == "FCEV"), 
            aes(label = value), 
            vjust = -1.5, color = "red", size = 3.5)+
  theme_minimal()
...
### Canada_electricity_production
```{r}
Canada_electricity_production <- energy_production %>% 
  filter(Country=="Canada",Product=="Electricity") %>% 
  select(-X) %>% 
  ...
  group_by(Time) %>% 
  summarize(Country = "Canada",Total_electricity_year = sum(energy_production),Unit = 'GWh') %>% 
  rename("year"="Time")

Canada_electricity_production %>% ggplot(aes(x = year, y = Total_electricity_year)) +
  geom_line(color = "lightseagreen", size = 1) +      
  geom_point(size = 2, color = "red") +     
  geom_text(aes(label = round(Total_electricity_year, 1)), vjust = -0.5, size = 3) +  
  labs(title = "Canada's Annual Energy Production (2010-2021)", 
       x = "Year", 
...
### Canada_ev_chargers
```{r}
Canada_ev_chargers <- ev_chargers %>% 
  filter(region=="Canada") %>% 
...
Canada_ev_chargers %>% 
  ...
  geom_line()+
  labs(title = "Canada's Charging Stations by Powertrain (Slow vs Fast)",
       x = "Year",
       y = "Number of Charging Stations",
       fill = "Powertrain",
       color = "Powertrain") +
  theme_minimal()+
  geom_text(data = Canada_ev_chargers %>% filter(powertrain == "Publicly available fast"),
            aes(label = value), vjust = -0.5, color = "red", size = 4)
```

### Merge
```{r}
Canada_merged <-Canada_ev_data_by_sale %>% 
  left_join(Canada_ev_chargers,by=c("Country","year")) %>% 
  full_join(Canada_electricity_production,by=c("Country","year")) %>% 
...
```{r}
Canada_metric <- Canada_merged %>% 
  group_by(year) %>% 
  reframe(total_ev_sales_per_year = sum(`# of EV`),
            total_charging_points_per_year = sum(`# of charging station`) ) %>% 
  mutate(total_electricity_per_year = Canada_electricity_production$Total_electricity_year,
         influence_metric = total_electricity_per_year * total_ev_sales_per_year / total_charging_points_per_year)

write.csv(Canada_metric,"~/Desktop/Data Challenge/Data Challenge/CODE/Metrics/Canada_metric.csv",row.names = FALSE)
```
...
```{r}
ggplot(filter(Canada_merged, `EV type` == "BEV" | `EV type` == "PHEV"), 
       aes(x = `# of charging station`, y = `# of EV`,color = `EV type`)) +
  geom_point() +
  geom_smooth(method = "lm", se = T,color="orange") + # choose linear method temporarily
  labs(title = "EV Sales vs Charging Stations (Canada)", x = "Number of Charging Stations", y = "EV Sales") +
  theme_minimal()
```
...
ggplot(filter(Canada_merged, `EV type` == "BEV" | `EV type` == "PHEV"), 
  ...
  labs(title = "EV Sales vs Electricity Production (Canada)", 
...
```{r}
cor_matrix <- cor(filter(Canada_merged, `EV type` == "BEV" | `EV type` == "PHEV") %>% select(`# of EV`, `# of charging station`, Total_electricity_year), use = "complete.obs")
print(cor_matrix)
```

# Regression Analysis
```{r}
model <- lm(`# of EV` ~ `# of charging station` + Total_electricity_year, data = filter(Canada_merged, `EV type` == "BEV" | `EV type` == "PHEV"))
summary(model)
```
